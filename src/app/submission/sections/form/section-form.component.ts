...
export class SubmissionSectionFormComponent extends SectionModelComponent {
  ...
  hasMetadataEnrichment(sectionData: WorkspaceitemSectionFormObject): boolean {
    ...
    Object.keys(sectionData).forEach((key) => {
      if (
        ...
        this.sectionMetadata.includes(key) &&
        this.inCurrentSubmissionScope(key)
      ) {
  ...
  private inCurrentSubmissionScope(field: string): boolean {
    ...
    const matchedRow = this.formConfig?.rows.find((row) => {
      return row.fields
        ? row.fields?.[0]?.selectableMetadata?.[0]?.metadata === field
        : [];
    });
    const scope = matchedRow ? matchedRow.fields?.[0]?.scope : '';
    /** kware end edit */
    switch (scope) {
