...
export function dsDynamicFormControlMapFn(
  ...
  switch (model.type) {
    ...
    case DYNAMIC_FORM_CONTROL_TYPE_RADIO_GROUP:
      return model instanceof DynamicListRadioGroupModel
        ? DsDynamicListComponent
        : DynamicNGBootstrapRadioGroupComponent;
  ...
  implements OnInit, OnChanges, OnDestroy
{
      ...
      if (hasValue(this.value) && this.value.isVirtual) {
        const relationship$ = this.relationshipService
          .findById(
            ...
            true,
            true,
            ...itemLinksToFollow(this.fetchThumbnail)
          )
   ...
   */
  onRemove(): void {
    /** kware start edit -- Fix adding and deleting entities relationships issues
     *  -- handel deleting un-rebeatable field relationship issue (un-rebeatable field not send any context)
     *     so, we force setting the formControl with undefined value.
     */
    if (this.context) {
      ...
      if (this.model.parent.context.groups.length === 0) {
        this.formBuilderService.addFormArrayGroup(
          formArrayControl,
          arrayContext
        );
      }
